    ...
    private _updateBoyArrow() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if(element.Id === this._currentBoyId) {
                ...
                nextName = i < (count - 1) ? bfList[i+1].Name : "";
                break;
            }
        }

        this.nodeNextBoy.active = index < (count-1);
        this.nodeLastBoy.active = index > 0;
        this.labLastBoyName.string = lastName;
        this.labNextBoyName.string = nextName;
        const lastBoyId = index > 0 ? bfList[index].Id : -1;
        const nextBoyId = index < (count-1) ? bfList[index + 1].Id : -1;
